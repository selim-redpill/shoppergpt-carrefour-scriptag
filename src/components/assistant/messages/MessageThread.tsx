...
export function MessageBubble({ message, showSender }: MessageBubbleProps) {
  ...
  return (
    <div class={`flex mb-2 md:mb-2.5 ${isUser ? "justify-end" : ""}`}>
      <div class={`flex flex-col w-fit max-w-[86%] md:max-w-[min(78%,42rem)] ${isUser ? "items-end" : "items-start"}`}>
        {!isUser && showSender && (
          <span class="block text-[18px] md:text-[20px] mb-2 md:mb-2.5 leading-none font-['Satisfy'] font-normal text-[#8B7A5A] tracking-[0.02em]">
            Cathia
          </span>
        )}
        <div
        ...
        <div class={`text-[10px] md:text-[11px] text-[#6B7280] mt-1.5 md:mt-2 px-1 ${isUser ? "text-right" : ""}`}>
    ...
    <div class="flex mb-2 md:mb-2.5">
      <div class="flex flex-col w-fit max-w-[86%] md:max-w-[min(78%,42rem)] items-start">
        ...
        <div class="inline-block w-fit max-w-full py-2 md:py-2.5 px-3 md:px-3.5 rounded-[4px_16px_16px_16px] md:rounded-[4px_18px_18px_18px] text-[13px] md:text-[13.5px] leading-[1.5] md:leading-[1.55] text-[#1A1A2E] shadow-[0_1px_3px_rgba(0,0,0,.06)] break-words bg-white">
          ...
          <span class="inline-block w-[2px] h-3 md:h-[13px] bg-[#8B7A5A] rounded-[1px] ml-0.5 align-middle animate-pulse" />
        </div>
      </div>
    </div>
  );
